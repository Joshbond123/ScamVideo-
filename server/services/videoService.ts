...
async function buildVideoSubtitles(jobId: string, lines: string[], durationSec: number) {
  const clean = lines.map((x) => String(x || '').replace(/\s+/g, ' ').trim()).filter(Boolean);
  if (!clean.length || durationSec <= 0) return '';

  const assPath = path.join(process.cwd(), 'database/assets/videos', `${jobId}.ass`);
  const perLine = Math.max(durationSec / clean.length, 0.5);
  const ass = [
    '[Script Info]',
    'ScriptType: v4.00+',
    'PlayResX: 1080',
  ...
  clean.forEach((line, i) => {
    const start = toAssTime(i * perLine);
    const end = toAssTime((i + 1) * perLine);
    ass.push(`Dialogue: 0,${start},${end},Viral,,0,0,0,,${line.replace(/,/g, '\\,').replace(/\{/g, '(').replace(/\}/g, ')')}`);
  });
...
async function assembleVideoLocally(jobId: string, audioPath: string, imagePaths: string[], subtitleLines: string[]) {
  ...
  const sceneDuration = Math.max(durationSec / imagePaths.length, 2.5);
  ...
  const assPath = await buildVideoSubtitles(jobId, subtitleLines, durationSec);
  const vf = assPath
    ? `scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,subtitles='${assPath.replace(/\\/g, '/').replace(/:/g, '\\:')}'`
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    proc.on('close', (code) => (code === 0 ? resolve() : reject(new Error(`ffmpeg failed (${code}): ${stderr}`))));
  });
...
export async function assembleVideo(jobId: string, audioPath: string, imagePaths: string[], subtitleLines: string[]) {
  try {
    ...
    throw new Error('Supabase render function did not return outputPath');
  } catch (error) {
